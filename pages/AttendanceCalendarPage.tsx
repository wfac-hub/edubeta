...
const AttendanceCalendarPage = () => {
    ...
    const handleStatusChange = (classId: string, newStatus: CourseClass['status']) => {
        ...
        if (classToUpdate) {
            // 1. Actualizar estado de la clase
            updateCourseClass({ ...classToUpdate, status: newStatus });

            // 2. Si se marca como 'Hecha', generar registros de asistencia para los alumnos inscritos si no existen
            if (newStatus === 'Hecha') {
                // Comprobar si ya existen registros para evitar sobrescribir
                const existingRecords = attendanceRecords.filter(r => r.classId === classId);
                
                if (existingRecords.length === 0) {
                    // Buscar alumnos inscritos en la fecha de la clase
                    ...
                    // Crear registros de asistencia por defecto (Presente)
                    enrolledStudents.forEach(enrollment => {
                         ...
                         if (student) {
                             updateAttendanceRecord({
                                 ...
                                 attended: true, // Por defecto asiste
                                 late: 'No',
