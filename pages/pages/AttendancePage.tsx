...
import { MoveLeft, RefreshCw, Search, Check, X, Mail, MessageSquare, Book, Users, UserCircle2, MoreHorizontal } from 'lucide-react';
...
const AttendancePage = () => {
    ...
    // Fix: Add state for the class whose comments are being edited.
    ...
    const enrolledStudents = useMemo(() => {
        // CORRECTO: Esta lógica asegura que solo se listen los alumnos que ya estaban inscritos en la fecha de la clase.
        if (!course || !courseClass) return [];
        const courseEnrollments = enrollments.filter(e => {
            if (e.courseId !== course.id || !e.isActive) {
                return false;
            }
            // Compara la fecha de inscripción con la fecha de la clase.
    ...
    useEffect(() => {
        if (classId) {
            const initialAttendance = enrolledStudents.map(student => {
                ...
                // Auto-create and save default record if none exists
    ...
    // Fix: Refactored the `filteredStudentIds` logic to use `Array.prototype.find` instead of a `Map`.
    // This ensures proper type inference for the 'student' object and resolves the 'property does not exist on type unknown' error.
    ...
    const handleCloseCommentModal = () => {
        ...
        // Fix: Reset commenting class state when closing the modal.
                            ...
                            <div key={student.id} className={`group ${!record.attended ? 'bg-red-50 dark:bg-red-900/20' : ''}`}>
                                 <div className="p-4 flex items-start gap-4" onClick={() => setExpandedRow(prev => prev === student.id ? null : student.id)}>
                                     ...
                                     {student.photoUrl ? (
                                        ...
                                        <div className="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-white text-sm" style={{backgroundColor: avatarColor}}>{initials}</div>
                                    )}
                                ...
                                <div className={`transition-all duration-300 ease-in-out overflow-hidden max-h-0 ${expandedRow === student.id ? 'max-h-[1000px]' : ''}`}>
                                    <div className="px-4 pb-4 pt-2 bg-gray-50 dark:bg-slate-700/50 text-sm space-y-4">
                                        ...
                                        {!record.attended && !record.absenceJustified && <div className="flex justify-between items-center"><span className="font-semibold">Avisar falta:</span> <div className="flex items-center gap-1"><Button variant="ghost" size="sm"><Mail size={16} /></Button><Button variant="ghost" size="sm"><MessageSquare size={16} /></Button></div></div>}
                                        ...
                                        {/* Fix: The `record` object does not have a `classInfo` property. Since the page is for a single class, the `courseClass` variable can be used directly. */}
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            {student.photoUrl ? (
                                                ...
                                                <div className="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-white text-sm" style={{backgroundColor: avatarColor}}>
                                                    {initials}
                                                </div>
                                            )}
