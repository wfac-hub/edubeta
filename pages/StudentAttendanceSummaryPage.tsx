...
import { MoveLeft, Search, Check, X, Mail, MessageSquare, Calendar, BookOpen, AlertCircle, Clock, ChevronDown } from 'lucide-react';
import CommentModal from '../components/CommentModal';

/**
 * Página que muestra el resumen de asistencias de un alumno específico.
 * Permite filtrar y editar rápidamente los registros de asistencia.
 * Estilos optimizados para día/noche con alto contraste.
 */
function StudentAttendanceSummaryPage() {
     ...
     * Filtra aquellos que no corresponden al periodo de inscripción.
     */
    const studentAttendanceRecords = useMemo(() => {
        if (!student) return [];
        
        // Obtener las inscripciones del alumno para validar fechas
        ...
        return attendanceRecords
            ...
            .filter(ar => {
                // 1. Debe tener información de clase
                if (!ar.classInfo) return false;

                // 2. Encontrar la inscripción correspondiente a este curso
                const enrollment = studentEnrollments.find(e => e.courseId === ar.classInfo!.courseId);
                if (!enrollment) return false; // Si no hay inscripción para el curso, no mostrar (datos huérfanos)

                // 3. Validar fechas (comparación string YYYY-MM-DD para evitar problemas de timezone)
                const classDateStr = ar.classInfo.date.toISOString().split('T')[0];
                const enrollmentDateStr = enrollment.enrollmentDate; // Ya es YYYY-MM-DD

                // Estrictamente ocultar si la clase es ANTERIOR a la inscripción
                if (classDateStr < enrollmentDateStr) return false;

                if (enrollment.cancellationDate) {
                    const cancellationDateStr = enrollment.cancellationDate; // Ya es YYYY-MM-DD
                    // Ocultar si la clase es POSTERIOR a la baja
                    if (classDateStr > cancellationDateStr) return false;
                }
    ...
    /**
     * Filtra los registros de asistencia según los criterios de búsqueda.
     */
    const filteredRecords = useMemo(() => {
        return studentAttendanceRecords.filter(record => {
            ...
            const dateFromMatch = filters.dateFrom ? record.classInfo!.date >= new Date(filters.dateFrom) : true;
            const dateToMatch = filters.dateTo ? record.classInfo!.date <= new Date(filters.dateTo) : true;
            ...
            <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 ml-14">
                        Histórico detallado de asistencia y comportamiento en clase.
                    </p>
                </div>
