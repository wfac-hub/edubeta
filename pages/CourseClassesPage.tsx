...
const CourseClassesPage = () => {
    ...
    const handleStatusChange = (classId: string, newStatus: CourseClass['status']) => {
        ...
        if (classToUpdate) {
            ...
            // 2. If marked as 'Hecha', generate attendance records if missing
            if (newStatus === 'Hecha') {
                 const existingRecords = attendanceRecords.filter(r => r.classId === classId);
                 if (existingRecords.length === 0) {
                     const enrolledStudents = enrollments.filter(e => {
                         if (e.courseId !== classToUpdate.courseId || !e.isActive) return false;
                         
                         const enrollmentDate = new Date(e.enrollmentDate);
                         enrollmentDate.setUTCHours(0, 0, 0, 0);
                         const classDate = new Date(classToUpdate.date);
    ...
    const formatDate = (c: CourseClass) => {
        ...
        return `${dayOfMonth}/${month}/${year} ${dayOfWeek} ${c.startTime}-${c.endTime}`;
    };
                            ...
                            {filteredClasses.map(c => {
                                ...
                                return (
                                    <tr key={c.id} className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600/50">
                                        ...
                                        <td className="px-6 py-4"><select value={c.status} onChange={(e) => handleStatusChange(c.id, e.target.value as CourseClass['status'])} className={`p-1 border rounded-md text-sm font-medium appearance-none focus:outline-none focus:ring-2 focus:ring-primary-500 ${statusColors[c.status]}`} onClick={(e) => e.stopPropagation()}><option value="Pendiente">Pendiente</option><option value="Hecha">Hecha</option><option value="Anulada">Anulada</option></select></td>
