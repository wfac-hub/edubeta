...
/**
 * Componente reutilizable para un filtro de selección (dropdown).
 */
...
/**
 * Página para gestionar los cursos activos.
 * Permite ver, crear, editar, duplicar y borrar cursos, así como gestionar su estado y horarios.
 */
const CoursesPage = () => {
    ...
    /**
     * Inicia el proceso para cambiar el estado de activación de un curso.
    ...
    /**
     * Confirma y ejecuta el cambio de estado de activación.
     */
    ...
    /**
     * Inicia el proceso de borrado. Analiza los cursos seleccionados para determinar
     * si pueden ser borrados (no tienen alumnos activos).
     * Muestra un modal de información si hay cursos no borrables, o un modal de confirmación si son borrables.
     */
    ...
    /**
     * Confirma y ejecuta el borrado de los cursos seleccionados.
     */
    ...
    /**
     * Cierra todos los modales relacionados con el borrado.
     */
    const closeModals = () => {
    ...
    /**
     * Abre el modal del formulario para crear un nuevo curso (`course` es `null`) o editar uno existente.
     */
    ...
    /**
     * Guarda los cambios de un curso (creación o edición) y cierra el modal.
     */
    ...
    /**
     * Abre el modal para seleccionar los horarios de un curso.
     */
    ...
    /**
     * Guarda los horarios seleccionados para un curso.
     */
    ...
    // Funciones auxiliares para obtener nombres y objetos relacionados.
    ...
    /**
     * Memoiza la lista de cursos filtrados por el término de búsqueda.
     * Solo muestra los cursos activos en esta página.
     */
    const filteredCourses = useMemo(() => {
        return courses.filter(course => {
            if (!course.isActive) return false;
            
            // FIX: Allow secondary teachers to see courses
            if (isTeacher && (course.teacherId !== user?.id && course.secondaryTeacherId !== user?.id)) {
                return false;
            }

            return (
                course.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                getTeacherName(course.teacherId).toLowerCase().includes(searchTerm.toLowerCase())
            );
        });
    }, [searchTerm, courses, teachers, isTeacher, user]);

    /**
     * Maneja la selección de todos los cursos filtrados.
     */
    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        setSelectedCourses(e.target.checked ? filteredCourses.map(c => c.id) : []);
    };

    /**
     * Maneja la selección/deselección de un único curso.
     */
    ...
    /**
     * Maneja el clic en el botón "Modificar", abriendo el formulario para el único curso seleccionado.
     */
    const handleModify = () => {
        if (selectedCourses.length === 1) {
            const courseToEdit = courses.find(c => c.id === selectedCourses[0]);
            if (courseToEdit) handleOpenFormModal(courseToEdit);
        }
    };

    /**
     * Maneja el clic en el botón "Duplicar", abriendo el formulario con los datos del curso seleccionado.
     */
        ...
        if (selectedCourses.length === 1) {
            ...
            if (courseToDuplicate) {
                const newCourse: Course = {
                    ...courseToDuplicate,
                    id: 0, // El ID 0 indica al DataContext que es una creación.
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Cursos activos</h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        Lista de todos los cursos activos en la actualidad. Puedes consultar el histórico de cursos en el menú Gestión Centro.
                    </p>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 space-y-4">
                <div className="flex flex-wrap items-center gap-2">
                    {/* Botones de acción - Hidden for Teacher */}
                    {!isTeacher && (
                        <>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {!isTeacher && <FilterSelect label="Profesor/a" options={teachers.map(t => ({ value: t.id.toString(), label: `${t.name} ${t.lastName}` }))} />}
                    {!isTeacher && <FilterSelect label="Recibos y facturación" options={[]} />}
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
                {/* Vista móvil (tarjetas expandibles hasta LG) */}
                <div className="lg:hidden divide-y divide-gray-200 dark:divide-slate-700">
                    ...
                    {filteredCourses.map((course) => {
                        ...
                        return (
                            <div key={course.id} className={`group ${!course.isActive ? 'bg-red-50 dark:bg-red-900/20 opacity-70' : ''}`}>
                                <div className="p-4 flex items-start gap-4" onClick={() => setExpandedRow(prev => prev === course.id ? null : course.id)}>
                                    ...
                                    <div className="flex-grow">
                                        <button onClick={(e) => { e.stopPropagation(); if(!isTeacher) handleOpenFormModal(course); }} className={`font-medium ${!isTeacher ? 'text-blue-600 dark:text-blue-400 hover:underline' : 'text-gray-900 dark:text-white cursor-default'} text-left`}>
                                            {course.name}
                                        </button>
                                ...
                                <div className={`transition-all duration-300 ease-in-out overflow-hidden max-h-0 ${expandedRow === course.id ? 'max-h-[500px]' : ''}`}>
                                    <div className="px-4 pb-4 pt-2 bg-gray-50 dark:bg-slate-700/50 text-sm space-y-2 border-t dark:border-slate-700">
                                        ...
                                        {!isTeacher && <div className="flex justify-between"><span className="font-semibold">Horarios:</span> <span>{course.scheduleIds.length}</span></div>}
                ...
                {/* Vista de escritorio (tabla visible desde LG) */}
                        ...
                        <table className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-slate-700 dark:text-gray-300">
                                <tr>
                                    ...
                                    <th scope="col" className="px-2 py-3 w-8"></th> {/* New column for Debt Icon */}
                                    ...
                                    <th scope="col" className="px-6 py-3 hidden lg:table-cell">Modalidad</th>
                                    <th scope="col" className="px-6 py-3">Aula</th>
                                    {!isTeacher && <th scope="col" className="px-6 py-3 hidden lg:table-cell">Horarios</th>}
                                    <th scope="col" className="px-6 py-3">Clases</th>
                                    <th scope="col" className="px-6 py-3 hidden xl:table-cell">Recur.</th>
                                    <th scope="col" className="px-6 py-3 hidden xl:table-cell">Fecha inicio curso</th>
                                    <th scope="col" className="px-6 py-3 hidden xl:table-cell">Fecha fin curso</th>
                                    {!isTeacher && <th scope="col" className="px-6 py-3">¿Activo?</th>}
                            ...
                            <tbody>
                                {filteredCourses.map((course) => {
                                    ...
                                    return (
                                        <tr key={course.id} className={`border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600/50 ${!course.isActive ? 'bg-red-50 dark:bg-red-900/20 opacity-70' : 'bg-white dark:bg-slate-800'}`}>
                                            <td className="w-4 p-4">
                                                <div className="flex items-center gap-2">
                                                    <input
                                                        ...
                                                        className="h-4 w-4 rounded border text-primary-600 focus:ring-primary-500 bg-white dark:bg-slate-900 border-gray-300 dark:border-slate-700"
                                                    />
                                                    {!isTeacher && <button onClick={() => handleOpenFormModal(course)} className="text-gray-400 hover:text-primary-600"><Pencil size={14} /></button>}
                                                </div>
                                            </td>
                                            ...
                                            <th scope="row" className="px-6 py-4 font-medium text-blue-600 dark:text-blue-400 whitespace-nowrap">
                                                {isTeacher ? (
                                                    <span className="text-gray-800 dark:text-white">{course.name}</span>
                                                ) : (
                                            ...
                                            <td className="px-6 py-4 hidden lg:table-cell">{course.modality}</td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                            ...
                                            <td className="px-6 py-4 hidden xl:table-cell">{new Date(course.startDate).toLocaleDateString()}</td>
                                            <td className="px-6 py-4 hidden xl:table-cell">{new Date(course.endDate).toLocaleDateString()}</td>
                                            {!isTeacher && (
                                                <td className="px-6 py-4">
