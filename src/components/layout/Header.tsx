...
const Header: React.FC<HeaderProps> = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const handleConnectToSupabase = async () => {
      // Intentar conectar directamente sin redirecciones
      ...
      try {
          ...
          if (!sb || !sb.url || !sb.anonKey) {
             // Si faltan credenciales, mostramos error pero no redirigimos forzosamente,
             // el usuario verá el estado de error en el botón.
                    ...
                    {dbConfig.provider === 'supabase' ? (
                        ...
                        <div className="flex items-center gap-2">
                            <button 
                                ...
                                title="Intentar conexión con Supabase"
                            >
                                {isConnecting ? <Loader size={12} className="animate-spin"/> : <LinkIcon size={12} />}
                                <span>{statusMessage || 'Conectar Supabase'}</span>
                            </button>
                        </div>
                    )}
                </li>
