...
const StudentForm: React.FC<StudentFormProps> = ({ student, onSave, onClose }) => {
    const { populations, academyProfile, addStoredFile } = useData();
    ...
    const formatDateForInput = (dateString?: string | Date | null) => {
        ...
        try {
            ...
            // If it's a string, try to handle ISO format or YYYY-MM-DD
            if (typeof dateString === 'string') {
                // If it already looks like YYYY-MM-DD
                if (/^\d{4}-\d{2}-\d{2}$/.test(dateString)) {
                    return dateString;
                }
                // Try splitting by T if it's ISO
                if (dateString.includes('T')) {
                     return dateString.split('T')[0];
                }
                // Attempt to parse any other format
    ...
    const [formData, setFormData] = useState<Student>(() => {
        if (student) {
             return {
                 ...
                 domiciliationData: student.domiciliationData ? {
                     ...student.domiciliationData,
                     acceptanceDate: formatDateForInput(student.domiciliationData.acceptanceDate)
                 } : undefined
             } as Student;
        }
    ...
    const handlePhotoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onloadend = () => {
            ...
            // Guardar automáticamente en el gestor de archivos
            addStoredFile({
                 ...
                 relatedId: formData.id || 0, // Si es nuevo, el ID podría ser 0 inicialmente, pero se asociará luego si se guarda
                 centerId: 1,
