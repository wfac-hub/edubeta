...
export class DataService {
    ...
    public async create<T>(tableName: string, item: T): Promise<T | null> {
        if (this.config.provider === 'supabase' && this.supabase) {
            ...
            // Si el ID es 0 o negativo (temporal), lo excluimos para que Supabase genere uno.
            // Si es un string (como en PDFs o IDs manuales), lo mantenemos.
            const payload = (typeof id === 'number' && id <= 0) ? rest : item;
