...
import { Student, AttendanceRecord, LateType, CourseClass, Role } from '../types';
import Button from '../components/ui/Button';
import { MoveLeft, RefreshCw, Search, Check, X, Mail, MessageSquare, Book, Users, UserCircle2, MoreHorizontal, EyeOff, ArrowRightCircle, ShieldAlert } from 'lucide-react';
...
const AttendancePage = () => {
    ...
    useEffect(() => {
        if (classId && courseClass) {
            const initialAttendance = enrolledStudents.map(student => {
                ...
                // If record doesn't exist, create it lazily
                // Determine status based on class status
                let status: 'Pendiente' | 'Realizado' | 'Anulado' = 'Pendiente';
                if (courseClass.status === 'Hecha') status = 'Realizado';
                else if (courseClass.status === 'Anulada') status = 'Anulado';

                const defaultRecord: AttendanceRecord = {
                    id: `${student.id}-${classId}`,
                    classId: classId!,
                    studentId: student.id,
                    attended: courseClass.status === 'Hecha', // If done, default attended=true
                    late: 'No' as const,
                    absenceJustified: false,
                    homeworkDone: courseClass.status === 'Hecha', // Assume HW done if attended
                    comments: '',
                    status: status
                };
    ...
    }, [enrolledStudents, attendanceRecords, classId, courseClass, updateAttendanceRecord]);
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'Realizado':
                return <span className="px-2 py-1 rounded text-xs bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border border-green-200 dark:border-green-800">Realizado</span>;
            case 'Anulado':
                return <span className="px-2 py-1 rounded text-xs bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 border border-red-200 dark:border-red-800">Anulado</span>;
            default:
                return <span className="px-2 py-1 rounded text-xs bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300 border border-yellow-200 dark:border-yellow-800">Pendiente</span>;
        }
    };
            ...
            <div className="flex flex-wrap justify-between items-center gap-4">
             ...
             <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
                <div className="flex flex-wrap items-center gap-2">
                     {/* UPDATED BACK BUTTON: Navigate to calendar preserving the class date */}
                    <Button 
                        variant="secondary" 
                        size="sm" 
                        leftIcon={<MoveLeft size={16} />} 
                        onClick={() => navigate(`/attendance-calendar?date=${courseClass.date.toISOString().split('T')[0]}`)}
                    >
                        Volver
                    </Button>
                            ...
                            <div key={student.id} className={`group ${!record.attended ? 'bg-red-50 dark:bg-red-900/20' : ''}`}>
                                ...
                                <div className={`transition-all duration-300 ease-in-out overflow-hidden max-h-0 ${expandedRow === student.id ? 'max-h-[1000px]' : ''}`}>
                                    <div className="px-4 pb-4 pt-2 bg-gray-50 dark:bg-slate-700/50 text-sm space-y-4">
                                        {/* Admin Status for Mobile */}
                                        {isAdmin && (
                                            <div className="flex justify-between items-center border-b border-gray-200 dark:border-slate-600 pb-2 mb-2">
                                                <span className="font-semibold flex items-center gap-1"><ShieldAlert size={14}/> Estado (Admin):</span>
                    ...
                    <table className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-slate-700 dark:text-gray-300">
                            <tr>
                                ...
                                {isAdmin && <th scope="col" className="px-6 py-3 text-center">Estado</th>}
