...
const GenerateInvoicesWizardPage = () => {
    ...
    const lastEmittedInvoice = useMemo(() => {
        // Check if series object exists to show next number
        const seriesObj = availableSeries.find(s => s.code === serie);
        if (seriesObj && seriesObj.nextNumber) {
            return `Próxima: ${seriesObj.nextNumber}`;
        }
        ...
        return `Última fecha: ${new Date(lastDate).toLocaleDateString()}`;
    }, [invoices, serie, availableSeries]);
                        ...
                        <div className="p-6 border border-gray-200 dark:border-slate-600 rounded-md bg-gray-50 dark:bg-slate-700/30">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{lastEmittedInvoice}</p>
                                </div>
                            </div>
                        </div>
