...
const SeriesForm: React.FC<{ series: Partial<BillingSeries> | null, onSave: (s: BillingSeries) => void, onClose: () => void }> = ({ series, onSave, onClose }) => {
    const [formData, setFormData] = useState<Partial<BillingSeries>>(series || { year: new Date().getFullYear(), code: '', isRectifying: false, isActive: true, nextNumber: 1 });
    
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const { name, value, type, checked } = e.target;
        let val: any = value;
        if (type === 'checkbox') val = checked;
        else if (type === 'number' || name === 'year' || name === 'nextNumber') val = parseInt(value, 10);
    ...
    return (
        <form onSubmit={(e) => { e.preventDefault(); onSave(formData as BillingSeries); }} className="space-y-4">
            ...
            <div>
                <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Siguiente número</label>
                <input type="number" name="nextNumber" value={formData.nextNumber || 1} onChange={handleChange} className="w-full p-2 border rounded bg-white dark:bg-slate-900 border-gray-300 dark:border-slate-600" required min="1" />
                <p className="text-xs text-gray-500 mt-1">El número que se asignará a la próxima factura de esta serie.</p>
            </div>
...
export default function BillingCenterSeriesPage({ center }: { center: BillingCenter }) {
    const { updateBillingSeriesState, deleteBillingSeries, invoices } = useData();
    ...
    const handleSave = (series: BillingSeries) => {
        updateBillingSeriesState({ ...series, centerId: center.id });
                ...
                <table className="min-w-full text-sm text-left">
                    <thead className="bg-gray-50 dark:bg-slate-700/50 text-xs uppercase font-medium text-gray-500 dark:text-gray-400">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-center">Siguiente Nº</th>
                        ...
                        {seriesList.map(s => {
                            ...
                            return (
                                <tr key={s.id} className={`hover:bg-gray-50 dark:hover:bg-slate-700/30 ${!s.isActive ? 'bg-red-50 dark:bg-red-900/10' : 'bg-white dark:bg-slate-800'}`}>
                                    ...
                                    <td className="px-6 py-4 text-center text-gray-600 dark:text-gray-400">{s.nextNumber}</td>
