        ...
        <div className="max-w-5xl mx-auto pb-24">
            ...
            <CollapsibleSection title="Paso 4 - Forma de cobro">
             ...
             <CollapsibleSection title="Email de notificación">
                 <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Configura las plantillas de correo electrónico que se enviarán automáticamente.</p>
                 <div className="space-y-6">
                     <div>
                     ...
                     {/* NOTIFICACIÓN ALUMNO NUEVO */}
                     <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-md border border-blue-100 dark:border-blue-800">
                        <h4 className="font-bold text-blue-800 dark:text-blue-200 mb-4 text-sm">Notificación de nueva inscripción (Alumno Nuevo)</h4>
                        <div className="space-y-4">
                            <div>
                                <label className={labelClasses}>Asunto del e-mail</label>
                            ...
                            <div>
                                <label className={labelClasses}>Plantilla de E-mail</label>
                                <div className="flex gap-2">
                                    <select 
                                        value={formData.notifications.confirmationTemplateId || ''} 
                                        onChange={e => handleDeepChange('notifications.confirmationTemplateId', parseInt(e.target.value))} 
                                        className={inputClasses}
                                    >
                                        <option value="">-- Seleccionar Plantilla --</option>
                                        {emailTemplates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                                    </select>
                                    <Button variant="secondary" onClick={() => handlePreviewEmail(formData.notifications.confirmationTemplateId, formData.notifications.confirmationSubject)} title="Vista Previa"><Eye size={16}/></Button>
                                </div>
                            </div>
                        </div>
                     </div>

                     {/* NOTIFICACIÓN ALUMNO EXISTENTE */}
                     <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-md border border-green-100 dark:border-green-800">
                        <h4 className="font-bold text-green-800 dark:text-green-200 mb-4 text-sm">Notificación de inscripción (Alumno Existente)</h4>
                        <div className="space-y-4">
                            <div>
                                <label className={labelClasses}>Asunto del e-mail</label>
                             ...
                             <div>
                                ...
                                <div className="flex gap-2">
                                    <select 
                                        value={formData.notifications.existingStudentTemplateId || ''} 
                                        onChange={e => handleDeepChange('notifications.existingStudentTemplateId', parseInt(e.target.value))} 
                                        className={inputClasses}
                                    >
                                        <option value="">-- Seleccionar Plantilla --</option>
                                        {emailTemplates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                                    </select>
                                    <Button variant="secondary" onClick={() => handlePreviewEmail(formData.notifications.existingStudentTemplateId, formData.notifications.existingStudentSubject)} title="Vista Previa"><Eye size={16}/></Button>
                                </div>
                            </div>
                        </div>
                     </div>
            ...
            <Modal isOpen={isPreviewModalOpen} onClose={() => setIsPreviewModalOpen(false)} title="Vista Previa del Email">
                <div className="bg-white dark:bg-slate-800 p-4 rounded-md border border-gray-200 dark:border-slate-700">
                    ...
                    <div>
                        ...
                        <div className="prose prose-sm max-w-none dark:prose-invert p-4 bg-gray-50 dark:bg-slate-900 rounded-md" dangerouslySetInnerHTML={{__html: sanitizeHTML(previewContent?.body || '')}} />
                    </div>
                </div>
